...
import { DeepSeekClient, ChatCompletionResponse } from '../src/deepseek/client.js';
...
describe('DeepSeekCodingAgent with Mock API', () => {
  ...
  beforeEach(() => {
    ...
    fs.writeFileSync(path.join(tmpDir, 'package.json'), JSON.stringify({ name: 'mock-app' }));
  ...
  it('executes agent loop with parallel tool calls and emits events and visible messages', async () => {
            ...
            choices: [
              {
                ...
                message: {
                  role: 'assistant',
                  content: 'I will list the workspace files and read package.json.',
                  tool_calls: [
                    {
                      id: 'call_1',
                      type: 'function',
          ...
          choices: [
            {
              ...
              message: {
                role: 'assistant',
                content: 'I have finished inspecting the project. The package name is mock-app.',
              },
              finish_reason: 'stop',
            },
          ],
    ...
    const completes = emittedEvents.filter((e) => e.type === 'action.completed');
