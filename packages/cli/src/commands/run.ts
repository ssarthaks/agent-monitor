...
export async function runAgentCommand(options: RunCommandOptions): Promise<void> {
  ...
  const approvalManager = new ApprovalManager({
    ...
    onApprovalRequested: async (approval: ApprovalRequest) => {
      ...
      console.log(`  ${pc.bold('Action:')}     ${pc.cyan(approval.actionKind)}`);
      if (approval.params.command) {
        console.log(`  ${pc.bold('Command:')}    ${pc.white(approval.params.command)}`);
      }
      if (approval.params.path) {
        console.log(`  ${pc.bold('Path:')}       ${pc.white(approval.params.path)}`);
      }
      console.log(`  ${pc.bold('Risk Score:')} ${pc.red(`${approval.risk.score}/100 (${approval.risk.level})`)}`);
      console.log(`  ${pc.bold('Policy:')}     ${pc.yellow(approval.matchedPolicies.join(', ') || 'custom rule')}`);
    ...
    onApprovalResolved: async (approval, decision, resolvedBy) => {
      ...
      const alreadyEmitted = existingEvents.some(
        (e) => e.type === 'approval.resolved' && (e as any).approvalId === approval.id
      );
  ...
  const summary: SessionEndedEvent['summary'] & { approvedCount?: number; blockedCount?: number } = {
